...
elif [ "$extension" = "grpc" ]; then
  tag=$(curl -sSL https://pecl.php.net/rest/r/grpc/allreleases.xml | grep -m 1 -Eo "([0-9]+.[0-9]+.[0-9]+)")
  sed -i '' "s/.*tgz.*/  url \"https\:\/\/pecl.php.net\/get\/grpc-$tag.tgz\"/g" ./Formula/"$version".rb
